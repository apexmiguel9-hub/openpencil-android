//! Floating VariablesPanel press dispatch on the web host — hit-test +
//! delegation only (#21: the wasm build gets the same interactive grid as
//! the desktop).
//!
//! Every arm body lives in the shared
//! `op_editor_core::host_variables_transitions` (the native host's twin
//! drives the same functions); row-cell presses stay host-side in
//! `variables_panel_rows.rs` and the draft commits in
